...
from PySide6.QtGui import QImage, QKeyEvent, QStandardItem, QStandardItemModel
...
class ItemImage(QStandardItem):
    def __init__(
        self, image_path: str, page_data: Dict[str, str], thumbnail_size: int
    ) -> None:
        super().__init__()
...
class PagesListStore(QStandardItemModel):
    def __init__(self, thumbnail_size: int, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)

        self.thumbnail_size = thumbnail_size

    def add_page(self, image_path: str, page_data: Dict[str, str]) -> None:
...
class PagesIconView(QListView):
    ...
    def __init__(
        ...
        model = PagesListStore(
            self.settings_manager.get(
                "thumbnail_size", self.settings_manager.get("thumbnail_size", 150)
            ),
            self,
        )
        self.setModel(model)
    ...
    def page_selected(self, index: QModelIndex) -> None:

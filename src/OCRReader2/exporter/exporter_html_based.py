...
class ExporterHTMLBased(Exporter):
    ...
    def get_page_content(self, export_data: Dict[str, Any]) -> str:
        html_content = ""
        for box_data_entry in export_data["boxes"]:
            added_content, new_section = self.get_box_content(
                box_data_entry, export_data["image_path"]
            )
    ...
    def get_box_content(
        self, box_data_entry: Dict[str, Any], image_path: Optional[str] = None
    ) -> Tuple[str, bool]:
        match box_data_entry["type"]:
            ...
            case _:
                logger.warning(f"Unknown box type: {box_data_entry['type']}")
        return ("", False)

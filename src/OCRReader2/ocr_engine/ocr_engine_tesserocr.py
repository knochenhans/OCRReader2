...
from tesserocr import (  # type: ignore
    PSM,
    RIL,
    PyPageIterator,
    PyTessBaseAPI,
...
from OCRReader2.ocr_engine.ocr_result import (
    OCRResultBlock,
    OCRResultLine,
    OCRResultParagraph,
    OCRResultSymbol,
...
def parse_ocr_results(ri: PyPageIterator) -> List[OCRResultBlock]:
...
def perform_ocr(
    api: PyTessBaseAPI, box: OCRBox, settings: Optional[SettingsManager] = None
) -> OCRBox:
    try:
        if isinstance(box, TextBox):
            ...
            if api.Recognize():
                ...
                if len(results) == 1:
                    # parse_ocr_results returns OCRResultBlock instances, so use the first result directly
                    block = results[0]
                    box.confidence = block.confidence
                    box.ocr_results = block
...
class OCREngineTesserOCR(OCREngine):
    def __init__(self, project_settings: Optional[SettingsManager] = None) -> None:
        ...
        for _ in range(NUM_THREADS):
            api = PyTessBaseAPI(lang=lang, path=self.path)
    ...
    def detect_orientation_script(self, image_path: str) -> Dict[str, Union[int, str]]:
        ...
        try:
            ...
            os: Dict[str, Union[int, str]] = api.DetectOS()
